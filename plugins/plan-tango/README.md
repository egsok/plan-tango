...
- Phase A heads-up (контракт перед run)
- Phase B lock-acquired подтверждение
...
```
Phase A. Validation
   resolve plan-path → validate (size, location) → resolve repo-root → heads-up

Phase B. Init
   acquire lock (session_id) → write state.json → ensure workspace dir
...
Phase D. Final (если status=converged*)
   pre-gate → Opus final-check (или diagnostic mode для non-converged + --force) →
   при critical/major: corrective iter → ОДИН Codex re-review

Phase E. Summary
   print stats → release lock → opt cleanup workspace
```
...
Bash(codex --version)                                   # version check at Phase A step 3
