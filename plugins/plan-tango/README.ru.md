...
Вызывается как **`/plan-tango:tango`** (или выбери из дропдауна slash-команд при наборе `/plan-tango`).
...
| `--final-check` | off | Opt-in: запустить Opus sanity-check на converged-статусах. Ставит `final_check="always"` для этого прогона. |
...
# Если `claude plugin path` недоступен, путь обычно: ~/.claude/plugins/marketplaces/plan-tango/plugins/plan-tango/skills/tango/user-config.example.json
# отредактируй
```
...
Валидация при загрузке: `effort` enum, `max_iter ≤ 12`, `thread_mode ∈ {fresh, continue}`, `service_tier ∈ {null, fast, flex}`, `final_check ∈ {never, always}` (старые значения `auto` / `force` **удалены в 0.7.0** — теперь loader hard-error'ит на них, называя замену). На нарушении — abort с понятной ошибкой ДО старта прогона.
...
**`--lenient` НЕ skip'ает Opus final-check** — `converged-lenient` остаётся eligible для Phase D pre-gate. Opus запускается только по opt-in (`--final-check` или `final_check: "always"`); дефолтный `final_check: never` уже его пропускает.
...
CLI: `/plan-tango:tango <slug> --quiet`
...
```
Затем: `/plan-tango:tango <slug> --codex-profile review-fast`.
...
- **Убрана детекция off-plan по упоминаниям.** Эвристика, которая сканировала предлагаемые Codex'ом правки на пути к чужим файлам, за все сессии зафлагала 14 findings — 14 false positives, 0 настоящих. Реальная гарантия была не в эвристике: скилл в принципе конструирует `Edit` только против plan-файла. Статус `off-plan-target` и ledger-action `off_plan_blocked` удалены.
- **Pre-flight предупреждение о неправильном worktree.** Init теперь проверяет, сколько относительных путей из плана реально существуют под resolved repo root; если большинство отсутствует — предупреждает и спрашивает, прежде чем сжечь целый прогон из неправильного worktree.
- **Детерминированные helpers для стопа и bookkeeping.** `evaluate-stop.mjs` вычисляет стоп-условие, а `commit-iter.mjs` атомарно обновляет state/lock после итерации (в одной реальной сессии ручная версия дважды пушила findings_history).
- **Более мягкий парсинг вердикта.** Парсер вердикта/findings теперь терпит markdown-оформление и строку вердикта в пределах первых нескольких непустых строк.
- **Breaking:** deprecated-алиасы `final_check` `auto` / `force` (и флаги `--no-final-check` / `--force-final-check`) теперь hard-error'ят вместо тихой миграции — переведи конфиг на `never` / `always`.
...
| `final-check-advisory` | Opus polish-only (Phase D 28a-polish) — advisory list, без apply |
| `final-check-ignored` | Legacy: Opus нашёл только minor/nit — пропустили |

| `action` | Когда |
|---|---|
| `applied` | Edit прошёл, план изменён |
| `deferred` | apply-fixes не смог применить (конфликт якоря / ambiguity / Edit отклонён) |
| `manual` | Codex предложил несколько вариантов |
| `advisory` | Polish-only finding — попадает в §6, не применяется |
...
**Diagnostics:** при подозрении на проблемы (codex CLI not found, plans dir not writable, lock stuck) запусти `node ${CLAUDE_PLUGIN_ROOT}/skills/tango/scripts/doctor.mjs` — проверит codex CLI, парсинг user-config, write-доступ к `~/.claude/plans/`, lock acquire/release цикл и контракт `run-codex-review.mjs` на bad input. Все проверки read-only / dry-run; пробные файлы убираются автоматически. Добавь `--json` для машинно-читаемого вывода.
...
**Альтернатива** (если не хочешь править глобальные settings): запускай `/plan-tango:tango` **вне** plan mode. Plan mode для скилла избыточен — план уже написан, дальше только review loop. Обычный режим с `defaultMode: "acceptEdits"` даёт silent flow без дополнительных настроек.
...
- Проверь: `node "${CLAUDE_PLUGIN_ROOT}/skills/tango/scripts/lock.mjs" inspect --slug <slug>` (или абсолютный путь под ~/.claude/plugins/marketplaces/plan-tango/...)
...
**"Most plan file refs are missing under `<repo_root>`"** — pre-flight обнаружил, что больше половины относительных путей, на которые ссылается план, не существуют под resolved repo root. Обычно это значит, что прогон запущен из неправильного worktree. Прерви и перезапусти из правильного repo root — либо продолжай, если план легитимно ссылается на ещё не созданные файлы.
...
└── plugins/plan-tango/
    ...
    └── skills/tango/
        ...
        │   ├── lock.mjs                      # lease-lock с session_id (host + dead-PID stale)
        │   ├── apply-fixes.mjs               # pure planner (auto/deferred/manual)
        │   ├── evaluate-stop.mjs             # детерминированная оценка стоп-условия (Phase C)
        │   └── commit-iter.mjs               # детерминированный post-iteration bookkeeping (state + lock)
        └── references/
