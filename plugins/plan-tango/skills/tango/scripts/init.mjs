...
function main() {
  ...
  if (!planPath) {
    if (args.resume) {
      failNoLock(
        "resume_no_plan",
        "Cannot --resume without an explicit plan path/slug or active plan. Re-run /plan-tango:tango <slug-or-path> --resume to be unambiguous."
      );
    }
    failNoLock(
      "no_plan_resolved",
  ...
  if (codexRes.status !== 0) {
    failNoLock(
      "codex_cli_missing",
      "Codex CLI not found on PATH. Install with `npm install -g @openai/codex`, then run `codex login` (or `/codex:setup`). Re-run /plan-tango:tango once codex --version succeeds."
    );
  }
